...
- [ ] `run_and_refresh` duplicated between `log.lua` and `bookmark.lua` — move to `init.lua`
...
- [ ] `gR` bound manually in `diff.lua` but via `setup_view_keymaps` everywhere else — inconsistent
